...
namespace Reflo
{
    public static class RefactorEngine
    {
        ...
        {
            var rules = typeof(IRefactorRule).Assembly
                .GetTypes()
                .Where(t => typeof(IRefactorRule).IsAssignableFrom(t) && !t.IsInterface && !t.IsAbstract);
        ...
        public static void ApplyRuleByName(string ruleName, string projectPath)
        {
            var ruleType = typeof(IRefactorRule).Assembly
                .GetTypes()
                .FirstOrDefault(t => t.Name.Equals(ruleName, StringComparison.OrdinalIgnoreCase));

            if (ruleType == null)
            {
                Console.WriteLine($"❌ Rule '{ruleName}' not found.");
                return;
            }

            var rule = (IRefactorRule)Activator.CreateInstance(ruleType)!;
            var files = Directory.GetFiles(projectPath, "*.cs", SearchOption.AllDirectories);

            ApplyRule(rule, files);
        }
